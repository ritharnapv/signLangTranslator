...
export default function TimelineRoadmap() {
  ...
  return (
    <div className="bg-white border border-[#ecece0] rounded-3xl p-6 shadow-sm" id="roadmap-card">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-[#2d2d28] font-sans tracking-tight">30-Day Development Roadmap</h2>
            <p className="text-xs text-[#7a7a6a]">Interactive design sequence from foundational API mockups to certified real-time ASL scanning</p>
          </div>
        </div>
        <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-[#f0f2ee] text-[#52a447] rounded-full text-xs font-semibold border border-[#e0e4db]">
          <span className="w-1.5 h-1.5 bg-[#52a447] rounded-full animate-ping"></span>
          Day 1 Live
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            return (
              <button
                ...
                className={`w-full text-left p-4 rounded-2xl border transition-all duration-300 flex items-start gap-4 ${
                  isActive
                    ? "bg-[#f0f2ee] border-[#7c8d7c] text-[#2d2d28] shadow-sm"
                    : "bg-[#fdfcf9] border-[#ecece0]/80 hover:bg-[#f0f2ee]/50 hover:border-[#7c8d7c]/40 hover:scale-[1.01]"
                }`}
              >
                <div className="mt-1">
                  {milestone.status === 'completed' ? (
                    <CheckCircle2 className="w-5 h-5 text-[#52a447]" />
                  ) : milestone.status === 'active' ? (
                    <div className="relative">
                      <span className="absolute inline-flex h-full w-full rounded-full bg-[#7c8d7c] opacity-60 animate-ping"></span>
                      <Circle className="w-5 h-5 text-[#7c8d7c] relative fill-[#f0f2ee]" />
                    </div>
                  ) : (
                    <Clock className="w-5 h-5 text-[#9a9a8a]" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className={`text-[10px] font-bold font-mono tracking-widest ${
                      milestone.status === 'active' ? "text-[#7c8d7c]" : "text-[#7a7a6a]"
                    }`}>
                      DAY {milestone.day}
                    </span>
                    <span className="text-[9px] text-[#5a6b5a] font-semibold bg-[#e0h-min bg-[#e0e4db]/40 px-2 py-0.5 rounded border border-[#e0e4db]/85">
                      {milestone.focusArea}
                    </span>
                  </div>
                  <h3 className={`text-sm font-semibold truncate ${isActive ? "text-[#2d2d28]" : "text-[#4a4a40]"}`}>
                    {milestone.title}
                  </h3>
                </div>
                <ChevronRight className={`w-4 h-4 self-center transition-transform ${isActive ? "text-[#7c8d7c] translate-x-1" : "text-[#9a9a8a]"}`} />
              </button>
            );
          })}
        </div>
        ...
        <div className="lg:col-span-7 flex flex-col justify-between bg-[#fdfcf9] border border-[#e8e4db] rounded-2xl p-5" id="milestone-details">
          <div>
            <div className="flex items-center justify-between mb-3 border-b border-[#ecece0] pb-3">
              <div>
                <span className="text-[10px] font-bold text-[#7c8d7c] tracking-wider font-mono">
                  MILESTONE DETAILS • DAY {selectedDay.day}
                </span>
                <h3 className="text-base font-bold text-[#2d2d28] mt-1 font-sans">
                  {selectedDay.title}
                </h3>
              </div>
              <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${
                selectedDay.status === 'completed'
                  ...
                  ? "bg-[#f0f2ee] text-[#7c8d7c] border border-[#e0e4db] animate-pulse"
                  : "bg-neutral-100 text-[#9a9a8a] border border-neutral-200"
              }`}>
                {selectedDay.status.toUpperCase()}
              </span>
            </div>

            <p className="text-xs text-[#5c5c50] leading-relaxed font-sans mb-4">
              {selectedDay.description}
            </p>

            <span className="text-[10px] font-bold text-[#7a7a6a] uppercase tracking-widest font-mono block mb-2">
              Deliverables Checkpoint:
            </span>
            <ul className="space-y-2 text-xs text-[#4a4a40]" id="roadmap-tasks">
              {selectedDay.tasks.map((task, idx) => (
                <li key={idx} className="flex items-start gap-2.5">
                  ...
                  <span className={selectedDay.status === 'completed' ? "line-through text-[#7a7a6a]" : ""}>{task}</span>
                </li>
              ))}
            </ul>
          </div>

          <div className="mt-5 pt-3 border-t border-[#ecece0] flex items-center gap-2 text-[#7a7a6a] text-[11px] font-sans">
